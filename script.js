...
document.querySelector(".hover-right").addEventListener("mouseover", () => scrubTo(scrub.vars.totalTime + spacing));
document.querySelector(".hover-left").addEventListener("mouseover", () => scrubTo(scrub.vars.totalTime - spacing));
document.querySelector(".hover-right").addEventListener("click", () => scrubTo(scrub.vars.totalTime + spacing));
document.querySelector(".hover-left").addEventListener("click", () => scrubTo(scrub.vars.totalTime - spacing));
...
function buildSeamlessLoop(items, spacing) {
	let overlap = Math.ceil(1 / spacing), // number of EXTRA animations on either side of the start/end to accommodate the seamless looping
		...
		loopTime = (items.length + overlap) * spacing + 0.5, // the spot at the end where we loop back to the startTime 
	...
	gsap.set(items, {xPercent: 400, opacity: 0,	scale: 1});
	...
	for (i = 0; i < l; i++) {
		...
		item = items[index];
		time = i * spacing;
		rawSequence.fromTo(item, {scale: 1, opacity: 0}, {scale: 1, opacity: 1, zIndex: 100, duration: 0.5, yoyo: true, repeat: 1, ease: "power1.in", immediateRender: false}, time)
